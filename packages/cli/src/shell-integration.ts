...
export const installShellIntegration = async (
  ...
  if (shell === "fish") {
    ...
    if (current !== null) throw new Error("Refusing to overwrite a modified IsAIokay.com Fish integration file.");
...
export const uninstallShellIntegration = async (
  ...
  if (shell === "fish") {
    ...
    if (current !== renderShellIntegration(shell)) throw new Error("Refusing to remove a modified IsAIokay.com Fish integration file.");
    await rm(target);
    return { path, changed: true };
  }
  const target = await resolvedStartupPath(path, true);
...
export const shellIntegrationInstalled = async (
  ...
  if (shell === "fish") return current === renderShellIntegration(shell);
  return markerRange(current) !== null;
};
