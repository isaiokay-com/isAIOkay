...
export const SHELL_CONTEXT_ENV = "ISAI_OKAY_SHELL_CONTEXT";
...
const contextMarkerFor = (shell: SupportedShell): string => shell === "fish"
  ? `set -gx ${SHELL_CONTEXT_ENV} $fish_pid`
  : shell === "powershell"
    ? `$env:${SHELL_CONTEXT_ENV} = [string]$PID`
    : `export ${SHELL_CONTEXT_ENV}=$$`;
...
export const renderShellIntegration = (shell: SupportedShell): string => {
  ...
  return `${START_MARKER}\n# Generated by isaiokay. Re-run \`isaiokay shell install\` to refresh.\n${guard}\n${activeMarkerFor(shell)}\n${contextMarkerFor(shell)}\n${functions}\n${guardEnd}\n${END_MARKER}\n`;
};

const compatibleShellIntegrations = (shell: SupportedShell): string[] => {
  const current = renderShellIntegration(shell);
  const withoutContext = current.replace(`${contextMarkerFor(shell)}\n`, "");
  return [current, withoutContext, withoutContext.replace(`${activeMarkerFor(shell)}\n`, "")];
};
...
export const installShellIntegration = async (
  ...
  if (shell === "fish") {
    ...
    if (current !== null && !compatibleShellIntegrations(shell).includes(current)) {
...
export const uninstallShellIntegration = async (
  ...
  if (shell === "fish") {
    ...
    if (!compatibleShellIntegrations(shell).includes(current)) {
...
export const getShellIntegrationStatus = async (
  ...
  if (shell === "fish") {
    const installed = compatibleShellIntegrations(shell).includes(current);
    return { installed, current: current === renderShellIntegration(shell) };
  }
  const range = markerRange(current);
